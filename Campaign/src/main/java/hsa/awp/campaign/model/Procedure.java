...
public class Procedure extends AbstractMandatorableDomainObject<Long> {
  ...
  /**
   * Creates a new {@link Procedure} and initializes it with appropriate values (e.g. inserts a HashSet at appropriate places, so
  ...
  // TODO All : check if the part of business logic can be located at a single point.
  // TODO All : check if we better use Calendar.clone() instead of holding the original reference.

  /**
   * Setter for the interval of a {@link Procedure}. If startDate is after endDate a {@link IllegalArgumentException} will be
   * thrown.
   *
   * @param startDate beginning of the {@link Procedure}.
   * @param endDate   end of the {@link Procedure}.
   */
  ...
  /**
   * Checks whether another {@link Procedure} is overlapping this {@link Procedure}. (same algorithm as in {@link Campaign})
   *
   * @param p {@link Procedure} to check.
  ...
  public boolean isOverlapping(Procedure p) {
    ...
    if (this.getStartDate().compareTo(p.getEndDate()) <= 0 && this.getEndDate().compareTo(p.getStartDate()) >= 0) {
      return true;
    }
    ...
    // proc.getEndDate().after(p.getEndDate())) {
    if (this.getStartDate().compareTo(p.getStartDate()) <= 0 && this.getEndDate().compareTo(p.getEndDate()) >= 0) {
      return true;
    }
