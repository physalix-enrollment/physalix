...
public abstract class AbstractDropAndSortableBox extends Panel {
  ...
  /**
   * Contructor which generates an empty priolist, with maxItems priolistItems.
   *
   * @param id       wicket id
   * @param maxItems number of items the priolist should hold
   */
   ...
   *
   * @param id       wicket id
   * @param events   eventlist with events to be displayed. list.size() should not
   *                 be grater than maxItems.
   * @param maxItems number of items the priolist should hold
   * @param isActive true if draggabiliy is given
   */
  public AbstractDropAndSortableBox(String id, List<Event> events, int maxItems, boolean isActive) {
    super(id);
    ...
    if (events != null) {
      if (events.size() > maxItems) {
        throw new IllegalArgumentException("list bigger than allowed items count");
      }
    ...
    draggablePrioListTargets = new ArrayList<DraggablePrioListTarget>(maxItems);
    ...
    itemContainers = new ListView<DraggablePrioListTarget>("DropAndSortableBox.list", draggablePrioListTargets) {
      ...
      protected void populateItem(ListItem<DraggablePrioListTarget> item) {
        ...
        draggablePrioListTargets.get(item.getIndex()).setOutputMarkupId(true);
  ...
  public void itemDropped(DragableElement element, DraggablePrioListTarget prioList, AjaxRequestTarget target) {
  ...
  private void addItemToElementList(DragableElement element,
    ...
    if (elements[index] == null) { // dropped on empty slot
      ...
      } else if (elements[index - 1] != null) { // dropped in empty slot && dropped in first slot. Just add element
        elements[index] = element;
      } else { // dropped in empty slot && previous slot is empty. move element up.
      ...
      for (int i = elements.length - 1; i > index; i--) { // move items down (last item is lost)
                                                          // until the index where the element was dropped
        if (i == elements.length - 1 && elements[i] != null) { // last element have to be moved back to item list
          AbstractPriorityListSelector pls = findParent(AbstractPriorityListSelector.class);
      ...
      elements[index] = element; // set element to position where it was dropped
    }
    updateAll(target);
  }
   ...
   *
   * @param target {@link AjaxRequestTarget} for the ajax update
   */
